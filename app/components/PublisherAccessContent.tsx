...
export function PublisherAccessContent({
  publisher,
}: {
  publisher: Publisher;
}) {
  ...
  /**
   * How the code should travel — asked BEFORE it exists.
   *
   * It used to be decided by the server: press «выдать код» and the letter was
   * already gone by the time the code appeared, after which the dialog offered
   * to write one. Nothing was broken, but the elder could not tell what had
   * happened, and writing that second letter made things worse — the one from
   * the server carries the login name and the install page, a hand-written one
   * carries neither.
   *
   * Null means the question is not being asked; there is nothing to ask when
   * the account has no address.
   */
  const [howToSend, setHowToSend] = useState(false);

  const resendMutation = useMutation({
    // Shown in place by this form — keep it out of the error strip.
    meta: { inlineError: true },
    mutationFn: (post: boolean) =>
      publishersApi.resendInvite(publisher.id, post),
    onSuccess: (summary) => {
      setIssued(summary);
      setHowToSend(false);
      invalidate();
  ...
  if (!access || !access.hasAccess) {
    return (
      <View>
        <Text style={styles.muted}>{t("publisherAccess.noAccess")}</Text>
        <Pressable style={styles.primaryBtn} onPress={() => setGrantOpen(true)}>
          <Text style={styles.primaryBtnText}>
            {t("publisherAccess.grant")}
          </Text>
        </Pressable>
        <GrantModal
          visible={grantOpen}
  ...
  return (
    <View>
      <View style={styles.row}>
        <Text style={styles.rowLabel}>{t("publisherAccess.loginNameRow")}</Text>
        <Pressable
      ...
      <View style={styles.row}>
        ...
        <Text style={styles.rowValue}>
          {formatLastLogin(access.lastLoginAt) ??
      ...
      {roleGrantsPrivate ? (
        ...
        <Text style={styles.hint}>{t("publisherAccess.privateDataDesc")}</Text>
      )}
      ...
      <Pressable
        ...
        onPress={() => {
          // No address, no question: the code can only be handed over.
          if (!access.email) resendMutation.mutate(false);
          else setHowToSend(true);
        }}
      ...
      >
        ...
        >
          {access.isActive
            ...
            : t("publisherAccess.enableAccess")}
        </Text>
      </Pressable>
...
function EmailModal({
  ...
  return (
    ...
    >
      ...
      {suggestion &&
        suggestion.trim() !== "" &&
        suggestion.trim().toLowerCase() !== (current ?? "").toLowerCase() && (
          <Pressable
      ...
      >
        ...
        {byMail ? (
          <View style={g.choiceBody}>
            <TextInput
              style={[g.input, address !== "" && !addressOk && g.inputBad]}
              value={email}
              onChangeText={setEmail}
              autoCapitalize="none"
              autoCorrect={false}
            ...
            {offerSave ? (
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={holders.length > 0 ? g.checkWarn : g.choiceHint}>
                    {holders.length > 0
    ...
    >
      ...
      */}
      ...
      {/*
        ...
        not.
      */}
      {/* Offered only when no letter went out. After one has, a second —
          written by hand, without the login name or the install page — is
          worse than the first and confusing besides. */}
      {sentTo ? null : (
        <Pressable
