  ...
  ru: {
    ...
    privacy: {
      ...
      blocks: [
        ...
        { p: 'Вы вправе получить доступ к своим данным, исправить их, удалить, ограничить обработку, получить их в переносимом виде, возразить против обработки и в любой момент отозвать согласие (без влияния на законность прошлой обработки). Запрос — на ' + EMAIL + '. Вы также вправе подать жалобу в надзорный орган (для NRW — LDI NRW). Экспорт своих данных и удаление учётной записи доступны прямо в приложении (Профиль → Мои данные); удаление обезличивает вашу запись, сохраняя организационные записи собрания.' },
  ...
  de: {
    ...
    privacy: {
      ...
      blocks: [
        ...
        { h: 'Ihre Rechte' },
        { p: 'Sie haben das Recht auf Auskunft, Berichtigung, Löschung, Einschränkung, Datenübertragbarkeit, Widerspruch und auf jederzeitigen Widerruf der Einwilligung (ohne Auswirkung auf die Rechtmäßigkeit der bisherigen Verarbeitung). Anfragen an ' + EMAIL + '. Es besteht ein Beschwerderecht bei einer Aufsichtsbehörde (für NRW: LDI NRW). Der Export Ihrer Daten und die Löschung Ihres Kontos sind direkt in der App möglich (Profil → Meine Daten); die Löschung anonymisiert Ihren Datensatz und erhält die organisatorischen Aufzeichnungen.' },
        { h: 'Minderjährige' },
  ...
  en: {
    ...
    privacy: {
      ...
      blocks: [
        ...
        { h: 'Your rights' },
        { p: 'You have the right to access, rectification, erasure, restriction, portability, objection, and to withdraw consent at any time (without affecting the lawfulness of prior processing). Requests to ' + EMAIL + '. You may lodge a complaint with a supervisory authority (for NRW: LDI NRW). You can export your data and delete your account directly in the app (Profile → My data); deletion anonymises your record while keeping the organisational records.' },
        { h: 'Minors' },
